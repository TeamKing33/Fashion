...
import styles from "./cssCompany/signup.module.css"
function Signin() {
...
//   axios.defaults.withCredentials =true;
  ...
  useEffect(() => {
    const loggedIn = Cookies.get('logincompany');
    if (loggedIn === 'true') {
      setLogin(true);
    }
  }, []);
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:8081/logincompany', formData);
      const responseData = response.data;
  
      if (responseData && responseData.Message === "Logged in successfully") {
        alert("Data is correct");
        setLogin(true);
        Cookies.set('logincompany', true, { expires: 1 / 24 });
